...
# This script should be run inside the VM after booting from golden-gnome.qcow2

set -e

echo "=== Creating Golden Snapshot ==="
echo "This will install all E2E dependencies and save a snapshot."
...
# 11. Install tmux (for terminal multiplexing in tests)
echo "11. Installing tmux..."
sudo dnf install -y tmux 2>/dev/null || true
...
echo "2. After reboot, save snapshot with QEMU monitor:"
echo "   virsh snapshot-create-as <vm-name> 'golden-deps' 'All dependencies pre-installed'"
echo ""
echo "Or use the E2E test with --snapshot flag to auto-save after setup."
