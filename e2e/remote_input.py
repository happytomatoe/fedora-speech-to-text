...
def main() -> int:
    ...
    def pointer_move(x, y):
        call(bus, RD_NAME, rd_path, RD_SESS_IFACE,
             "NotifyPointerMotionAbsolute", "(sdd)", stream_path, x, y)
    ...
    try:
        if cmd == "move":
            ...
            call(bus, RD_NAME, rd_path, RD_SESS_IFACE,
                 "NotifyPointerAxisDiscrete", "(ui)", AXIS_VERTICAL,
                 int(rest[0]))
        elif cmd == "keysym":
            kv = int(rest[0], 0)
            call(bus, RD_NAME, rd_path, RD_SESS_IFACE,
                 "NotifyKeyboardKeysym", "(ub)", kv, True)
            call(bus, RD_NAME, rd_path, RD_SESS_IFACE,
                 "NotifyKeyboardKeysym", "(ub)", kv, False)
        else:
