...
# Fixed-path model cache matching the workflow cache action
# (/home/runner/moonshine-model); the service runs with an isolated HOME whose
...
mkdir -p "$SHOT_DIR"
cp "$ASSETS/e2e/extensions/poc-screenshot/metadata.json" "$SHOT_DIR/"
cp "$ASSETS/e2e/extensions/poc-screenshot/extension.js" "$SHOT_DIR/"
...
mkdir -p "$INPUT_DIR"
cp "$ASSETS/e2e/extensions/e2e-input/metadata.json" "$INPUT_DIR/"
cp "$ASSETS/e2e/extensions/e2e-input/extension.js" "$INPUT_DIR/"
